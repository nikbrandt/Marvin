		...
		if (command == 'guild' || command == 'server' || command == 'g' || command == 's') {
			...
			sql.get(`SELECT * FROM guildOptions WHERE guildId = ${gID}`).then(row => {
				...
				if (args[0] == 's' || args[0] == 'setting' || args[0] == 'option' || args[0] == 'o') {
					if (args[1] != 'levels' && args[1] != 'list' && args[1] != 'l' && args[1] != 'prefix') return message.channel.send('Invalid argument. For a list, type .g s l');
					...
					sql.run(`UPDATE guildOptions SET ${args[1]} = '${args[2]}' WHERE guildId = ${gID}`);
					message.channel.send(`Setting \`${args[1]}\` updated to \`${args[2]}\``);
				}
			});
		}
	}
