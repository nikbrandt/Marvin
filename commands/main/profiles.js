		...
		if (command == 'profile' || command == 'profiles' || command == 'p') {
			...
			if (args[0] == '' || args[0] === undefined) {
				...
				sql.get(`SELECT * FROM profiles WHERE userId = '${usID}'`).then(row => {
					...
					const pSuffix = suffix.slice(args[0].length + args[1].length + 2);
			...
			sql.get(`SELECT * FROM profiles WHERE userId = '${usID}'`).then(row => {
				...
				function sendEmbed() {
					const pEmb = new Discord.RichEmbed()
				...
				if (row.image === null) {
					...
					request(requestS2, function (err, response, body) {
						...
						img2.src = body;
						const im2r = 385 / 75;
						const im2h = img2.width / im2r;
						const im2hh = img2.height / 2;
						let im2mh = 37.5;
						if (img2.height < 75) {
							im2mh = im2hh;
						}
