		...
		if (command == 'profile' || command == 'profiles' || command == 'p') {
			...
			if (args[0] == '' || args[0] === undefined) {
				...
				sql.get(`SELECT * FROM profiles WHERE userId = '${usID}'`).then(row => {
					...
					if (args[1].toLowerCase() == 'status' && pSuffix.length > 50) return message.channel.send('Statuses cannot be longer than 50 characters.');
					if (args[1].toLowerCase() == 'bio' && pSuffix.length > 300) return message.channel.send('Bios cannot be longer than 300 characters.');
					...
					if (args[1].toLowerCase() == 'websites' && pSuffix.length > 150) return message.channel.send('No more than 150 characters of websites, please.');
					if (args[1].toLowerCase() == 'contact' && pSuffix.length > 75) return message.channel.send('Contact info shouldn\'t use more than 75 characters');
					if (args[1].toLowerCase() == 'location' && pSuffix.length > 40) return message.channel.send('It shouldn\'t take more than 40 characters to describe your location');
